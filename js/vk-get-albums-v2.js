;(function () {
  ...
  function getAlbumImages(album, doSelect){
    ...
    vkApi('photos.get', { album_id: id }, function(data){
      ...
      if (img){
        albumThumbs.append('<div class="col-md-4 col-sm-4"><img data-id="'+id+'" src="'+img.src_big+'"/></div>')
  ...
  function selectAlbum(albumId){
    ...
    for(var i = 0; i<images.length; i++){
      html += '<div><img data-u="image" src2="'+images[i].src_xbig+'"/></div>'
    }
  ...
  $(function(){
    ...
    $(albumThumbs).on('click', 'img', function(event){
      selectAlbum($(event.target).data('id'))
    })
